<?php
	...
	$playcanvas_api = 'UPbPrpAkrH5A8qt8BVx4BuH4yrbuPpme';
	$project_id     = '674069';
	$primary_scene  = '1643571';
	$project_name   = 'Venge.io';
	$branch_id      = '2f72949b-728b-4a71-b9b8-007850b38cdb';
	...
	function progressBar($done, $total) {
	    ...
	    $write = sprintf("\033[0G\033[2K[%'={$perc}s>%-{$left}s] - $perc_number%% - $done/$total", "", "");
	    fwrite(STDERR, $write);
	}
	...
	//get scenes
	$scenes = json_decode(shell_exec('curl --silent -H "Authorization: Bearer '.$playcanvas_api.'" https://playcanvas.com/api/projects/'.$project_id.'/scenes?branchId='.$branch_id), true);
	...
	//request job
	$job = json_decode(shell_exec('curl --silent -H "Authorization: Bearer '.$playcanvas_api.'" -H "Content-Type: application/json" -X POST -d \'{"scenes" : ['.implode(',', $scenes).'], "scripts_concatenate":true, "optimize_scene_format" : true, "branch_id": "'.$branch_id.'", "scripts_minify" : true ,"project_id": '.$project_id.', "name": "'.$project_name.'"}\' https://playcanvas.com/api/apps/download'), true);
	...
	//query job
	$jobs = json_decode(shell_exec('curl --silent -H "Authorization: Bearer '.$playcanvas_api.'" https://playcanvas.com/api/jobs/'.$job['id']), true);

	//give at least 5 seconds to finish job
	sleep(5);

	if($jobs['status'] == 'running'){
		echo "⛔️ Waiting little bit more..."."\n";
		sleep(30);
	}
	...
	if($jobs['data']['download_url']){
		echo "✅ Got the file, downloading..."."\n";
	}

	$download = shell_exec('curl --silent -O '.$jobs['data']['download_url']);

	if(!file_exists($project_name . '.zip')){
		echo '⛔️ Zip file doesnt exist!'."\n";
		var_dump($jobs);
		exit;
	}

	$unzip    = shell_exec('unzip -o '.$project_name.'.zip -x "index.html" "__settings__.js"');
	$delete   = shell_exec('rm '.$project_name.'.zip');

	//replace index with origin
	$hash = md5(time());
	$hash = substr($hash, 0, 6);
	$current_version = file_get_contents('index.html');
	$current_version = preg_replace("/var VERSION = '(.*?)';/", "var VERSION = '".$hash."';", $current_version);

	$current_version = preg_replace("/var VERSION_CODE = '(.*?)';/", "var VERSION_CODE = '".$version_code."';", $current_version);

	$current_version = preg_replace("/playcanvas-stable\.min\.js\?v=(.*?)\"/", "playcanvas-stable.min.js?v=".$version_code."\"", $current_version);

	file_put_contents('index.html', $current_version);

	echo "✅ New deployment prepared : ".$hash."\n";



	echo '👌 '.$count." files uploaded!"."\n";
	echo "☕️ Deployment hash : ".$hash."\n";

	echo 'Concating the files'."\n";
	...
	$concat = '(function () {';
	$concat.= 'var $_x0 = window.pc;';
	$concat.= 'window.pc = pc = { FILLMODE_NONE : "FILLMODE_NONE", FILL_WINDOW : "FILL_WINDOW", FILLMODE_KEEP_ASPECT : "FILLMODE_KEEP_ASPECT"};';

	foreach ($files as $key => $file) {
		$content = file_get_contents($file);
		$content = str_replace('pc.', '$_x0.', $content);
		$content = str_replace('pc,', '$_x0,', $content);
		$content = str_replace('pc[', '$_x0[', $content);
		$content = str_replace("'pc'", "'$_x0'", $content);
		$concat .= $content;
	...
	file_put_contents('__game-scripts.js', $concat);
	...
	class BunnyAPI
	{
	    ...
	    public function purgePullZone($id, $db_log = 0)
	    {
	        ...
	        return $this->APIcall('POST', "pullzone/purgeCache/$id", json_encode(array("id" => $id)));
	    }
	...
	$bunny = new BunnyAPI();
	$bunny->apiKey($bunny_api);
	$bunny->zoneConnect($storage, $access_key);

	if($argv[3] == '--only-changes'){
		$count = 0;

		echo "Files listed will be updated : "."\n";
		$files = shell_exec('sudo git status -uno');
		$files = preg_match_all('/(modified|new file):   (.*?)\n/', $files, $matches);

		$total = count($matches[2]);

		foreach ($matches[2] as $key => $file) {
			$remotePath = $bunny_base . $file;

			if(!is_dir($sourcePath)){
				echo ('UPLOAD : '. $remotePath."\n");
				progressBar($count, $total);

				$bunny->uploadFile($file, $remotePath);
		...
		foreach ($files as $key => $file) {
			if(
				...
				strpos($file, '/README.MD') === false &&
				strpos($file, '/deploy.php') === false
			){
				$remotePath = $bunny_base . $file;
